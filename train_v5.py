...
def elastic_transform(image, severity=4):
    ...
    c = [(img_shape * 0.5, img_shape * 0.3, 0),
         ...
         (img_shape * 2, img_shape * 0.08, 0),
         (img_shape * 2, img_shape * 0.07, 5),
         (img_shape * 5, img_shape * 0.06, 10),
         ][severity]
    ...
    return rescale_intensity(map_coordinates(image, indices, order=1, mode='reflect').reshape(shape), out_range=(0, 255)).astype(int)
